    ...
    <Box sx={{ display: 'flex', width: '100vw', height: '100vh' }}>
      <Card sx={{ Width: '50%', Height: '50%', m: 1, display: 'flex', position: 'relative' }}>
        ...
        <CardContent>
          ...
          <Typography sx={{ fontSize: 14, fontStyle: 'italic' }} align='left'>
            {'Длительность фильма: ' +
              dataFilm.runtime +
              ' минут / ' +
              (dataFilm.runtime / 60).toFixed(2) +
              ' ч'}
          </Typography>
          <Typography sx={{ fontSize: 14, fontStyle: 'italic' }} align='left'>
            {'Жанр фильма: ' + dataFilm.genres.map((a: any) => a.name).join(', ')}
          </Typography>
          <Typography sx={{ fontSize: 14, fontStyle: 'italic' }} align='left'>
            {'Язык оригинала: ' + dataFilm.original_language}
          </Typography>
          <Typography sx={{ fontSize: 14, fontStyle: 'italic' }} align='left'>
            {'Выпускающая компания: ' + dataFilm.production_companies.map((a: any) => a.name).join(', ')}
          </Typography>
          <Typography sx={{ fontSize: 14, fontStyle: 'italic' }} align='left'>
            {'Страны производства ' + dataFilm.production_countries.map((a: any) => a.name).join(', ')}
          </Typography>
          <Typography sx={{ fontSize: 14, fontStyle: 'italic' }} align='left'>
